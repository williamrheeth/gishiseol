...
for epoch in range(num_epochs):
    ...
    for batch_index, (batch_X, batch_y, batch_biases) in enumerate(tqdm(train_dataloader, desc=f"Training Epoch {epoch+1}/{num_epochs}", leave=True)):
        ...
        denormalized_outputs.retain_grad()

        if batch_index % 100 == 0:
            # Prepare for position computation
            start_idx = batch_index * train_dataloader.batch_size
            end_idx = start_idx + batch_X.size(0)
            true_quaternions = quaternions_train[start_idx:end_idx].to(dtype=torch.float32, device=device)
            true_positions = positions_train[start_idx:end_idx].to(dtype=torch.float32, device=device)
            
            # Compute estimated positions using the predicted biases
            estimated_positions = []
            for i in range(batch_X.size(0)):
                initial_position = positions_all[train_idx[start_idx + i]].to(device)
                initial_velocity = velocities_all[train_idx[start_idx + i]].to(device)
                estimated_position = compute_acceleration_with_correction(
                    initial_position, 
                    batch_X[i, -1, 0:3],  # Last accelerometer input in the sequence
                    denormalized_outputs[i], 
                    initial_velocity, 
                    gravity_vector.to(device), 
                    true_quaternions[i], 
                    dt=dt, 
            ...
            # Log raw outputs, denormalized outputs, and positions
            print(f"Epoch [{epoch+1}/{num_epochs}], Batch [{batch_index+1}/{len(train_dataloader)}]:")
            print(f"Raw model output (scaled): {outputs[-1].cpu().detach().numpy()}")
            print(f"Denormalized model output (predicted bias): {denormalized_outputs[-1].cpu().detach().numpy()}")
            print(f"True bias: {batch_y[-1].cpu().detach().numpy()}")
        ...
        loss = criterion(true_quaternions, true_positions, denormalized_outputs, batch_X, positions_all, velocities_all, gravity_vector, train_idx[start_idx:end_idx], sequence_length)
        ...
        data_counter += batch_X.size(0)
        
        #if data_counter % 5000 < batch_X.size(0):  # Condition to check if it's time to print
            #predicted_position = compute_acceleration_with_correction(
            #    positions_train[start_idx], batch_X[-1, :, 0:3], outputs[-1], velocities_train[start_idx], gravity_vector, true_quaternions[-1]
            #)
            
            #print(f"\n--- Data point {data_counter} ---")
           # print(f"Predicted Biases: {outputs[-1].cpu().detach().numpy()}")
            #print(f"True Biases: {batch_y[-1].cpu().detach().numpy()}")
            
            #print(f"Predicted Position: {predicted_position.cpu().detach().numpy()}")
            #print(f"True Position: {true_positions[-1].cpu().numpy()}\n")
    ...
    with torch.no_grad():
        for batch_index, (batch_X, batch_y, batch_biases) in enumerate(tqdm(val_dataloader, desc="Validating", leave=True)):
            ...
            if batch_index % 10 == 0:
                ...
                estimated_positions = []
                for i in range(batch_X.size(0)):
                    initial_position = positions_all[val_idx[start_idx + i]].to(device)
                    initial_velocity = velocities_all[val_idx[start_idx + i]].to(device)
                    estimated_position = compute_acceleration_with_correction(
                        initial_position, 
                        batch_X[i, -1, 0:3],  # Last accelerometer input in the sequence
                        denormalized_outputs[i], 
            ...
            true_positions = positions_val[start_idx:end_idx].to(dtype=torch.float32, device=device)
            loss_quaternion = criterion(true_quaternions, true_positions, denormalized_outputs, batch_X, positions_all, velocities_all, gravity_vector, val_idx[start_idx:end_idx], sequence_length)
